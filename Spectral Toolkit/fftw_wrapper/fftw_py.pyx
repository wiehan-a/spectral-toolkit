...
def real_fft(np.ndarray[np.float64_t, ndim=1] x, N, threads=1):
    '''
...
def inverse_real_fft(np.ndarray[np.complex128_t, ndim=1] x, N, threads=1):
    '''
...
cdef inverse_real_fft_c(np.ndarray[np.complex128_t, ndim=1] x, N, threads=1):
    '''
    Performs the N-point inverse FFT of the real sequence x.
